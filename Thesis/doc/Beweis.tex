...
After having proved all the basic geometric facts, we will turn our attention to finally prove the main theorem.
\begin{theorem}
...
\end{todo}
We take an arbitrary point $P \in S$ and consider its tangent plane $T_P(S) = \V(h)$.
Of course, no plane lies in $S$ completely, as this would 

By restricting the equation of $S$ to the tangent plane we obtain a 3-form $f' = f - hg$, $g$ being some quadratic form.
If we're lucky $f'$ is reducible and hence a product of a linear and a quadratic form meaning that the intersection $T_P(S) \cap \V(h) \simeq \V(f') \subset \proj^2_k$ is a union of a line and a conic (possibly a degenerate one).
So in this case $S$ contains a line.
We proceed assuming that we are unlucky and say $f'$ is irreducible.
\begin{todo}
\item prove that intersecting with a tangent plane gives a singularity
\end{todo}
